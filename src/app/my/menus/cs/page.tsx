...
interface SiteFaqText {
    id: string;
    question: string;
    answer: string;
}

const faqs: Array<SiteFaqText> = [
    {
        id: '1',
...
export default function Page() {
    ...
    useEffect(() => {
        const getAndSubmitFaqList = async () => {
            try {
                const test = await basicUnpagedGet<Array<SiteFaqText>>(`/manage/admin/site_text`);
                if (!test || test?.length === 0) setQna(faqs);
                else setQna(test);
            } catch (error: any) {
                console.log('error', error)
                setQna(faqs as Array<SiteFaqText>);
            }
        };
